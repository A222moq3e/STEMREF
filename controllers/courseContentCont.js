...
module.exports = {
  ...
  courseContentByName: async (req, res) => {
    ...
    try {
      ...
      const reviewDocs = await reviewsCollection
        .find({ course: courseDoc._id })
        .populate("user", "name")
        .lean();
      const ratings = reviewDocs.map((r) => r.rating);
      const avg = ratings.length
        ? (ratings.reduce((a, b) => a + b, 0) / ratings.length).toPrecision(2)
        : 0;
  ...
  courseContentByNameAndCategory: async (req, res) => {
    ...
    try {
      ...
      if (!courseDoc) res.send("sorry this course not found");
      else {
        res.render("courseContent", {
          data: {
  ...
  courseContentRate: async (req, res) => {
    ...
    try {
      ...
      const avgVal = all.length
        ? (all.reduce((sum, r) => sum + r.rating, 0) / all.length).toPrecision(
            2,
          )
        : 0;
