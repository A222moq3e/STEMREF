...
module.exports = {
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      const data = await coursesCollection
        .find({ name: { $regex: sanitizedQuery, $options: "i" } })
        .sort(sortWay);
  ...
  EducatorDashboardGet: (req, res) => {
    ...
    res.render("EducatorDashboard", {
      data: { user: req.session.user, path: "/" + req.path.split("/")[1] },
    });
  },
  ...
  EducatorDashboardPost: async (req, res) => {
    ...
    try {
      ...
      const courseIsExist = await coursesCollection.findOne({
        name: data.name,
      });
      if (courseIsExist) {
